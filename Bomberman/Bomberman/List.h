...
public:
	...
	*/
	bool del(p2List_item<tdata>* item)
	{
		...
		if(item->prev != NULL)//If its the middle or the end
		{
			item->prev->next = item->next;

			if(item->next != NULL) //If it's in the middle
			{
		...
		else//The first one is removed
		{
			if(item->next)//If there's more nodes after the first one
			{
				item->next->prev = NULL;
				start = item->next;
			}
			...
			{
				start = end = NULL;//If there's only one node
			}
		}
